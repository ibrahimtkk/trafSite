...
def djangodanAl(request):
    if request.is_ajax():
        ...
        havaOlayliGunler = []
        distinctHavaOlayiYasananDegerler = mypytoncodeAnaliz.havaDurumuAjax(cursor, semt)
...
def home_to_havaDurumu(request):
    resimAdresi = mypytoncodeAnaliz.havaDurumu(request, cursor)[0]
