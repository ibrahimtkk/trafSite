...
def aylikOrtHizBul(request, cursor):
    vSegID = 444
    vSegDir = 0
    yil = 2017
    ...
    vSegID = haritadanEnYakinSensorBulLatLong(lat, lng)
    vSegDir = int(request.POST.get('vSegDirAylik'))
    print("-----------: " ,vSegID)
...
def havaDurumu(request, cursor):
    ...
    resimAdresi = hava.havaOlayiResimCikar(yil1, ay1, gun1, ay2, gun2, vSegID, vSegDir)
    return resimAdresi
...
def havaDurumuAjax(cursor, secilenSemt):
    ...
    havaOlayliDegerler, distinctHavaOlayiYasananDegerler = hava.havaOlayindanAyVeGunBulYagmur(secilenSemt)
