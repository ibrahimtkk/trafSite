...
# ==========================================================================
    def aylikOrtHizBul(self, yil, vSegID, vSegDir, cursor):
        ...
        plt.legend(['Ocak', 'Şubat', 'Mart', 'Nisan'], loc='best', prop={'size':15})
...
# ==========================================================================
    ...
    def havaOlayindanAyVeGunBulYagmur(self, secilenSemt):
        ...
        yagmurluGunler = []
        yagmurluAylar = []
        yields = df.loc[ (df['TOPLAM_YAGIS_OMGI_mm']>yagisSiniri) ]
        #print("degerler")
        #print(type(yields))
        ...
        ufakYagisListListesi = []
        for innerDegerler in distinctHavaOlayiYasananDegerler:
            ay, gun = innerDegerler[3], innerDegerler[4]
            ufakYagisList = df.loc[(df['TOPLAM_YAGIS_OMGI_mm']>ufakYagisSiniri) & (df['AY']==ay) & (df['GUN']==gun)]
            #print("???????: ", ufakYagisList)
            ufakYagisListListesi.append(ufakYagisList)
        ...
        return yields, distinctHavaOlayiYasananDegerler, ufakYagisListListesi
    ...
    def gunVeSensorVerCiktiAl(self, cursor, yil, ay, gun, vSegID, vSegDir):
        ...
        #print("-------------------------: ", yil, ay, gun, vSegID, vSegDir)
    ...
    def havaOlayiResimCikar(self, yil1, ay1, gun1, ay2, gun2, vSegID, vSegDir, ufakYagisListListesi, havaOlayliGunler):
        havaDurumuKlasor = 'C:\\Users\\ibrahim\\Desktop\\trafSite\\CSV\\analiz\\havaDurumu\\'
        #print(yil1, ay1, gun1, ay2, gun2, vSegID, vSegDir)
        ...
        #print("ufak------: ",type(ufakYagisListListesi))

        ilkListe, ikinciListe = [], []
        for ufakYagisList in ufakYagisListListesi:
            df = pd.DataFrame(ufakYagisList)
            #print("-+-+-+: ", df.loc[ (df['AY'].astype(float)==float(ay1)) & (df['GUN']==float(gun1))])
            #print("+-+-+: ", df.loc[ (df['AY'].astype(float)==float(ay2)) & (df['GUN']==float(gun2))])
            if not (df.loc[ (df['AY'].astype(float)==float(ay1)) & (df['GUN']==float(gun1))]).empty:
                ilkListe = df.loc[ (df['AY'].astype(float)==float(ay1)) & (df['GUN']==float(gun1))]
            if not (df.loc[ (df['AY'].astype(float)==float(ay2)) & (df['GUN']==float(gun2))]).empty:
                ikinciListe = df.loc[ (df['AY'].astype(float)==float(ay2)) & (df['GUN']==float(gun2))]
        #print(ilkListe)
        #print(ikinciListe)
        df1 = pd.DataFrame(ilkListe)
        df2 = pd.DataFrame(ikinciListe)
        yagmurluSaatler1, yagmurluSaatler2 = [], []
        for index, row in df1.iterrows():
            Istasyon_No, Istasyon_Adi, YIL, AY, GUN, SAAT, Yagis= row['Istasyon_No'], row['Istasyon_Adi'], row['YIL'], row['AY'], row['GUN'], row['SAAT'], row['TOPLAM_YAGIS_OMGI_mm']
            #print("111: ", SAAT)
            yagmurluSaatler1.append(SAAT)
        for index, row in df2.iterrows():
            Istasyon_No, Istasyon_Adi, YIL, AY, GUN, SAAT, Yagis= row['Istasyon_No'], row['Istasyon_Adi'], row['YIL'], row['AY'], row['GUN'], row['SAAT'], row['TOPLAM_YAGIS_OMGI_mm']
            #print("222: ", SAAT)
            yagmurluSaatler2.append(SAAT)

        #print(yagmurluSaatler1, yagmurluSaatler2)

        saatAraligi1, saatAraligi2 = [], []
        #if len(yagmurluSaatler1)!=0:
        #    saatAraligi1 = [yagmurluSaatler1[0]]
        #if len(yagmurluSaatler2)!=0:
        #    saatAraligi2 = [yagmurluSaatler2[0]]
        #print("-*-*-*-*-*-: ", saatAraligi1, saatAraligi2)

        i = 0
        fark = 1
        while (i<len(yagmurluSaatler1)-1):
            if i+fark >= len(yagmurluSaatler1):
                saatAraligi1.append(yagmurluSaatler1[i])
                saatAraligi1.append(fark)
                i = len(yagmurluSaatler1)
            else:
                if ( yagmurluSaatler1[i+fark]-yagmurluSaatler1[i]==1 ):
                    fark += 1
                else:
                    saatAraligi1.append(yagmurluSaatler1[i])
                    saatAraligi1.append(fark)
        ...
        i = 0
        fark = 1
        while (i<len(yagmurluSaatler2)-1):
            if i+fark >= len(yagmurluSaatler2):
                saatAraligi2.append(yagmurluSaatler2[i])
                saatAraligi2.append(fark)
                i = len(yagmurluSaatler2)
            else:
                if ( yagmurluSaatler2[i+fark]-yagmurluSaatler2[i]==1 ):
                    fark += 1
                else:
                    saatAraligi2.append(yagmurluSaatler2[i])
                    saatAraligi2.append(fark)
        ...
        #for i in range(len(yagmurluSaatler1)-2):
        #    if yagmurluSaatler1[i+1]-yagmurluSaatler1[i]!=1:
        #        saatAraligi1.append(yagmurluSaatler1[i])

        #for i in range(len(yagmurluSaatler2)-2):
        #    if yagmurluSaatler2[i+1]-yagmurluSaatler2[i]!=1:
        #        saatAraligi2.append(yagmurluSaatler2[i])
        ...
        ilkTarihString = str(gun1).replace('0', '')+"/"+str(ay1).replace('0', '')+"/"+str(yil1)
        ikinciTarihString = str(gun2).replace('0', '')+"/"+str(ay2).replace('0', '')+"/"+str(yil1)

        ilkTarihTersString = str(ay1).replace('0', '')+"/"+str(gun1).replace('0', '')+"/"+str(yil1)
        ikinciTarihTersString = str(ay2).replace('0', '')+"/"+str(gun2).replace('0', '')+"/"+str(yil1)

        if ilkTarihTersString in havaOlayliGunler:
            ilkTarihString += ' (yağmurlu)'
        if ikinciTarihTersString in havaOlayliGunler:
            ikinciTarihString += ' (yağmurlu)'
        if ilkTarihTersString not in havaOlayliGunler:
            ilkTarihString += ' (yağmursuz)'
        if ikinciTarihTersString not in havaOlayliGunler:
            ikinciTarihString += ' (yağmursuz)'
        ...
        plt.legend([ilkTarihString, ikinciTarihString], loc='best', prop={'size':15})
        print("*/*/*/*/*: ", saatAraligi1, saatAraligi2)
        print("-*/-*/-*/-*/-*/*/+++++ havaOlayliGunler:", havaOlayliGunler)
        for i in range(0, len(saatAraligi1), 2):
            x,y = self.range_brace(saatAraligi1[i], saatAraligi1[i]+saatAraligi1[i+1]-1)
            plt.plot(x, y,'-', color='r')
        for i in range(0, len(saatAraligi2), 2):
            x,y = self.range_brace(saatAraligi2[i], saatAraligi2[i]+saatAraligi2[i+1]-1)
            plt.plot(x, y,'-', color='g')
        #x,y = self.range_brace(8, 9)
        #plt.plot(x, y,'-')
        plt.grid()
