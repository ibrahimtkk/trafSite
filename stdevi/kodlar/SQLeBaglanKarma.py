...
class SQLeBaglanKarma():
    ...
    def vSegIDVerKoordinatAl(self, vSegID):
    ...
    def SiraliKoorVeHizDondur(self, semt, yil, ay, gun, vSegDir):
        ...
        for siraliSensor in siraliSensorler:
            ...
            for sensor in siraliSensor:
                koorList = self.vSegIDVerKoordinatAl(sensor)
                hiz = self.vSegIDVerHizAl(sensor, vSegIDlerVeHizlar)
                koorList.append(hiz)
                for koor in koorList:
                    gonderilecek[-1].append(koor)

        return gonderilecek


    def siraliListeBul(self, semtVeSensorlerList, semtIndex):
    ...
    ### [ [1, 30], [2, 31], ... ]
    def sensorlerinOrtHizBul(self, yil, ay, gun, vSegDir):
        ...
        with open('C:\\Users\\ibrahim\\Desktop\\trafSite\\CSV\\stdevi\\{}-{}-{}-{}-sensorlerVeHizOrt.csv'.format(yil, ay, gun, vSegDir), 'wb') as fp:
    ...
    def secilenSemttekiSensorlerinOrtHizlari(self, semt, yil, ay, gun, vSegDir):
        ...
        with open('C:\\Users\\ibrahim\\Desktop\\trafSite\\CSV\\stdevi\\{}-{}-{}-{}-sensorlerVeHizOrt.csv'.format(yil, ay, gun, vSegDir), 'rb') as fp:
